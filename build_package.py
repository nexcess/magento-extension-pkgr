...
PKGR_BIN_PHP and PKGR_BIN_XMLLINT environment variables. Useful for
...
class Magento_Packager(object):
    BIN_PHP = os.environ.get('PKGR_BIN_PHP', 'php')
    BIN_XMLLINT = os.environ.get('PKGR_BIN_XMLLINT', 'xmllint')
    BIN_BASH = os.environ.get('PKGR_BIN_BASH', 'bash')
    ...
    def get_syntax_check_results(self):
        ...
        results = [(syntax_map.get(os.path.splitext(filename)[1].lower(),
                unsupported_syntax_check)(filename), filename) \
            for filename in self._file_list]
        return results

    def do_syntax_check(self):
        final_result = True
        for result, filename in self.get_syntax_check_results():
            if not result:
